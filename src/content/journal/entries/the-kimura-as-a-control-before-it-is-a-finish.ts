import type { PublishedArticle } from "../types.ts";

/**
 * One grip, three set-ups. The organising claim is that the figure-four on the
 * wrist earns its place in a game as a control rather than as a finish, which
 * is why the same grip appears from positions that have nothing else in common.
 *
 * Deliberately does not attribute the technique's invention or its English name
 * to any individual. The naming question is left open in the body text and in
 * contestedNotes, because the accounts in circulation are not settled and this
 * site does not print gym history as fact.
 */
export const theKimuraAsAControlBeforeItIsAFinish: PublishedArticle = {
  status: "published",
  publishedAt: "2026-08-04",
  authorId: "steven-p",
  slug: "the-kimura-as-a-control-before-it-is-a-finish",
  category: "technique-notes",
  title: "The kimura as a control before it is a finish",
  standfirst:
    "The figure-four grip on a wrist is worth more for the arm it removes from the exchange than for the submissions it occasionally produces.",
  sections: [
    {
      id: "one-grip-several-names",
      heading: "One grip, several names",
      paragraphs: [
        "The grip is simple to describe and awkward to do well. Your hand takes the opponent's wrist, your other arm threads under their upper arm and takes your own wrist, and the resulting figure encloses their arm at two points. The shoulder is the joint it acts on. The elbow is where it is held.",
        "It has more names than most techniques. Judo's nomenclature places it in the ude-garami family, catch wrestling calls it a double wristlock, and most jiu-jitsu rooms call it a kimura. Where the last name came from is a story told confidently and told in several versions, and this article does not adjudicate between them. What matters here is that three traditions arrived at the same grip and used it differently, which is a hint about what it is for.",
        "What follows treats the grip as an object in its own right, appearing from half guard, from side control and from a front headlock. Those three positions share almost nothing. The grip is the same in all of them and does the same job in all of them, which is why it is worth learning as a unit rather than as three techniques that happen to look alike.",
      ],
    },
    {
      id: "what-the-grip-actually-does",
      heading: "What the grip actually does",
      paragraphs: [
        "Start with the claim, because it is the whole article. A figure-four on a wrist removes one of your opponent's arms from every other exchange happening at the same time. That is worth more, more often, than the submission is.",
        "Consider what an arm does in a scramble. It posts, it frames, it underhooks, it grips a wrist of yours, it catches the mat when the base goes. An arm inside a figure-four does none of those things, and it cannot be withdrawn without dealing with the grip first, which takes both attention and a hand. You have not only taken an arm; you have committed a second one to getting it back.",
        "This is why the grip travels between positions that otherwise have nothing in common. It is not a finish that happens to be available from several places. It is a control whose value does not depend on where you are standing, and the finish is one of the things you can do with it once you have it.",
        "It is also why so many people learn it badly. Taught as a submission, it produces the pattern where somebody grabs the grip, immediately tries to rotate the arm, loses it, and concludes the technique does not work on stronger opponents. Taught as a control, the first question is not how to finish but what the grip prevents and what that makes available next.",
      ],
    },
    {
      id: "from-half-guard",
      heading: "From half guard",
      paragraphs: [
        "From the bottom of half guard the grip solves a specific problem. The top player's near arm is either underhooking you or posting to stop the sweep, and both of those are arms you can reach.",
        "Take the figure-four on that arm and the top player's options collapse. They cannot post on it to stop being rolled, they cannot use it to flatten you, and if they try to pull it free they generally have to give up the pressure holding you down. In practice the grip is a sweep more often than it is a finish, because the moment they defend the arm, their base changes and the sweep is there.",
        "There is a second, quieter effect. Half guard is a position where the top player's chest and your chest are fighting over an inch or two of space, and an arm trapped in a figure-four is an arm that is not contributing to that fight. Reclaiming the underhook while somebody's arm is tied is a different task from reclaiming it while both of their arms work.",
        "The risk in this set-up sits with the sweep rather than with the lock. Rolling somebody while you hold their arm in a figure-four means their arm arrives at the mat in a position they did not choose, and their body follows. Go slowly, particularly on the rotation as they turn, and let go of the grip if the roll is happening faster than you can control the arm through it.",
      ],
    },
    {
      id: "from-side-control",
      heading: "From side control",
      paragraphs: [
        "From side control the grip is usually taken on the far arm, and the sequence people learn first is to step over the head to finish it. That step is the part worth being careful about, and it is worth understanding why the grip is useful even when you never take it.",
        "Held from side control, the figure-four stops the bottom player from framing with the far arm, from turning away, and from getting the far shoulder off the mat. Those are the three components of most escapes from underneath. A player who holds the grip and does nothing else has made the position substantially harder to leave, and has done it with a structure that does not fatigue in the way that shoulder pressure does.",
        "The finish is a rotation of the shoulder against a held elbow. This is where the specific injury risk of the technique lives, and it is worth naming precisely: the rotation has a short range and the attacker's body weight can move through it faster than a person can decide to tap, particularly when the finish is applied by stepping over and dropping rather than by turning the wrist gradually. Rotational loads are also easy to apply from a position where the person receiving them cannot see what is happening. Apply it slowly, in stages, and release completely on any tap without a final adjustment.",
        "The corresponding rule for the person underneath is to tap early rather than to test it. A shoulder held in a figure-four with somebody's weight above it has already made the point. This is a technique to be introduced by a coach and drilled with a cooperative partner before it appears in a live round, and the round should be one where the terms were agreed before it started.",
      ],
    },
    {
      id: "from-the-front-headlock",
      heading: "From the front headlock",
      paragraphs: [
        "The third set-up is the one that most clearly shows the grip working as a control. When somebody shoots and you meet them with a front headlock, one of their arms is often inside, near your hip, and reachable.",
        "Taken there, the figure-four is rarely a finish and is frequently everything else. It stops them building back up to their feet on that side, because the arm they would post with is committed. It gives you a handle to rotate them with, which is how the grip becomes a route to the back or to a change of angle. And it is a stalling point that costs them more than it costs you: your two hands against their one arm, in a position where they need both.",
        "That is the pattern to notice across all three set-ups. The grip's value is not that it threatens a shoulder. It is that it converts a symmetric exchange into an asymmetric one, and then lets you choose what to do with the asymmetry.",
        "It also generalises the safety rule. Anything that rotates a shoulder from a position where the other person cannot see it needs to be applied with less force than feels necessary, and abandoned rather than forced when the position moves. That applies more here than anywhere, because a front headlock is a scramble position and scrambles move.",
      ],
    },
    {
      id: "what-the-rulesets-permit",
      heading: "What the rulesets permit",
      paragraphs: [
        "Shoulder locks are broadly legal in adult submission grappling, and the shape of the exceptions is instructive. The ADCC rules list any arm bar, shoulder lock or wrist lock among legal techniques. The Unified Rules of MMA, defining small joint manipulation as a foul, state that fingers and toes are small joints while wrists, ankles, knees, shoulders and elbows are all large joints. United World Wrestling's grappling rules restrict guillotines, foot locks, calf and bicep slicers and knee bars in the under-13 and under-15 categories, and do not restrict shoulder locks there.",
        "The IBJJF's rule book takes the opposite approach to the same problem, listing prohibited holds in a table organised by age and belt division. The numbered items in that table include the omoplata and the wrist lock, which are the shoulder lock's near neighbours, along with a set of leg attacks and spinal locks. The table is a grid, the restrictions differ by division, and the only sensible instruction is to read the version that applies to you.",
        "Judo restricts joint attacks more narrowly than any of them. The IJF's rules describe the sport as one where, apart from the elbow joint, techniques are executed in the sense of the articulation and never in hyperextension. A shoulder lock does not sit inside that boundary, which is a reminder that the legality of a technique is a property of a document rather than of the technique.",
      ],
    },
    {
      id: "what-can-and-cannot-be-claimed",
      heading: "What can and cannot be claimed",
      paragraphs: [
        "Nothing in the research literature is about this grip. There is no measurement of how often a kimura control leads to a sweep, a back take or a finish, and no comparison between rooms that teach it as a control and rooms that teach it as a submission.",
        "The nearest relevant finding is indirect. Spanias, Kirk and Ovretveit, analysing 26 no-gi submission-only matches, found that time spent in dominant positions correlated with winning by an upper-body submission and did not correlate with winning by a lower-body one. Upper-body attacks in that sample arrived through position. A grip that improves position and threatens a shoulder sits on the same side of that line, which is consistent with the argument here and is not evidence for it.",
        "So the case for treating the kimura as a control is mechanical and it is testable in your own training rather than in a citation. Take the grip in a positional round and forbid yourself the finish. Count what becomes available. If the answer is nothing, the argument is wrong for the way you play, and that is a more useful result than a statistic somebody assembled from a different sport under a different ruleset.",
      ],
    },
  ],
  sources: [
    {
      title:
        "ADCC Rules and Regulations: legal techniques, including any arm bar, shoulder lock or wrist lock",
      publisher: "Abu Dhabi Combat Club",
      url: "https://adcombat.com/adcc-rules-regulations/",
      accessed: "2026-08-04",
    },
    {
      title:
        "United World Wrestling Grappling Rules 2025: illegal actions and the age restrictions for the under-13 and under-15 categories",
      publisher: "United World Wrestling",
      url: "https://cdn.uww.org/2025-05/grappling_rules_2025.pdf",
      accessed: "2026-08-04",
    },
    {
      title:
        "IBJJF Rule Book, version 6.1 (2024JUN): the table of technical fouls and illegal moves, organised by age and belt division",
      publisher: "International Brazilian Jiu-Jitsu Federation",
      url: "https://ibjjf.com/books-videos",
      accessed: "2026-08-04",
    },
    {
      title:
        "Unified Rules of Mixed Martial Arts, August 2025: foul 16, small joint manipulation, and the definition of large joints",
      publisher: "Association of Boxing Commissions and Combative Sports",
      url: "https://www.abcboxing.com/wp-content/uploads/2025/08/Unified-Rules-of-MMA-8.2025.pdf",
      accessed: "2026-08-04",
    },
    {
      title:
        "Sport and Organisation Rules of the International Judo Federation, version 12.03.2024, Appendix D: Referee Rules",
      publisher: "International Judo Federation",
      url: "https://78884ca60822a34fb0e6-082b8fd5551e97bc65e327988b444396.ssl.cf3.rackcdn.com/up/2024/04/IJF_SOR_version_12_03_2024_App-1712052995.pdf",
      accessed: "2026-08-04",
    },
    {
      title:
        "Charalampos Spanias, Christopher Kirk and Karsten Ovretveit, \"Position before submission? Techniques and tactics in competitive no-gi Brazilian jiu-jitsu\", Revista de Artes Marciales Asiaticas 17(2), 130-139 (2022)",
      publisher: "Sheffield Hallam University Research Archive (SHURA)",
      url: "https://shura.shu.ac.uk/31193/",
      accessed: "2026-08-04",
    },
  ],
  relatedSlugs: [
    "the-armbar-from-closed-guard",
    "the-guillotine-from-the-front-headlock",
    "grip-decay-and-the-half-life-of-a-no-gi-grip",
  ],
  contestedNotes: [
    "The origin of the English name kimura is told in several incompatible versions and no source consulted for this article settles it. The article names the technique's other names and declines to attribute its invention or its naming to any individual.",
    "The claim that the grip is more valuable as a control than as a finish is a mechanical argument. No study measures the outcomes of a kimura grip in any position, and none compares teaching approaches.",
    "The IBJJF illegal-moves table is a grid of divisions against numbered techniques. The column assignments could not be read reliably from the PDF text, so techniques are named as appearing in the table without a claim about which divisions they are prohibited for.",
    "The Spanias correlation between positional dominance and upper-body submissions comes from 26 matches at two no-gi submission-only events and is consistent with the argument here rather than evidence for it.",
  ],
};
